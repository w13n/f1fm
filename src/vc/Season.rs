...
impl Season {
    ...
    pub(crate) fn view(&self) -> Element<SeasonMessage> {
        let top_row = widget::row![
            widget::button("-").on_press_maybe((!self.current_round.eq(&1)).then_some(SeasonMessage::DecrementRound)),
        ...
        let mut leaderboard_vec: Vec<_> = leaderboard
            .into_iter()
            .collect::<Vec<_>>();
        leaderboard_vec.sort_by(|a, b| b.1.cmp(&a.1));

        let leadership_col: Vec<_> = leaderboard_vec.into_iter().map(|lp| text!("{:04}: {}", lp.1, lp.0).into()).collect();
        ...
        let bottom_row = widget::row![
            ...
            widget::button("download").on_press_maybe((!status.1).then_some(SeasonMessage::Download)),
            widget::button("score").on_press_maybe((!status.2).then_some(SeasonMessage::Score)),
        ];


        widget::column![top_row, widget::Column::from_vec(leadership_col), bottom_row].into()
    }
    ...
    pub fn update(&mut self, message: SeasonMessage) {
        match message {
            ...
            SeasonMessage::Draft => {
                self.season.draft(self.current_round, Box::new(Skipper::new())).unwrap()
            }
