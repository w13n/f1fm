...
impl Status {
    ...
    fn get_round_status(&self, round: u8) -> Option<&RoundStatus> {
    ...
    fn find_mut_round_status(&mut self, round: u8) -> &mut RoundStatus {
        if self.tasks.len() < round as usize {
    ...
    pub fn has_results(&self, round: u8) -> bool {
        if let Some(status) = self.get_round_status(round) {
    ...
    pub fn toggle_results(&mut self, round: u8) {
        let rs = self.find_mut_round_status(round);
